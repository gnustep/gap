...
#import "DBMassiveUpdater.h"
#import <DataBasinKit/DBSObject.h>
#import <DataBasinKit/DBSFTypeWrappers.h>
#import "DBTextFormatter.h"

NSString * const DBMUStatusKey = @"Status";

@implementation DBMassiveUpdater

- (id)init
{
  if ((self = [super init]))
    {
      winMassiveUpdater = nil;
      arrayRows = nil;
      updatedRows = nil;
      filteredRows = nil;
      sObj = nil;
...
- (void)awakeFromNib
{
  ...
  [searchField  setRecentsAutosaveName:@"MassiveUpdaterSearchRecents"];
  ...
  [winMassiveUpdater makeFirstResponder:fieldObjId];
}

- (void)show
{
  if (winMassiveUpdater == nil)
    [NSBundle loadNibNamed:@"MassiveUpdater" owner:self];

  [winMassiveUpdater makeKeyAndOrderFront:self];
}
...
- (void)resetUI:(NSDictionary *)statusDict
{
  ...
  statusMsg = [statusDict objectForKey:DBMUStatusKey];
  if (statusMsg)
    {
...
- (void)performLoadObject:(id)argObj
{
  ...
  if (objDevName == nil)
    {
      ...
      [self resetUI:[NSDictionary dictionaryWithObjectsAndKeys:@"Invalid object", DBMUStatusKey, nil]];
      [arp release];
      return;
    }
  ...
  NS_HANDLER
	...
	[faultTextView setString:[localException reason]];
        [faultPanel performSelectorOnMainThread:@selector(makeKeyAndOrderFront:) withObject:nil waitUntilDone:NO];
        [self resetUI:[NSDictionary dictionaryWithObjectsAndKeys:@"Exception", DBMUStatusKey, nil]];
        [arp release];
	return;
      }
  NS_ENDHANDLER
  ...
  [winMassiveUpdater setTitle: objDevName];
  [self resetUI:[NSDictionary dictionaryWithObjectsAndKeys:@"Loaded", DBMUStatusKey, nil]];
  [arp release];
}
...
- (void)performUpdateObject:(id)argObj
{
    ...
    if ([[localException name] hasPrefix:@"DB"])
      {
        ...
        [fieldNames release];
        [self resetUI:[NSDictionary dictionaryWithObjectsAndKeys:@"Update Error", DBMUStatusKey, nil]];
        [arp release];
        return;
      }
  NS_ENDHANDLER
  ...
  [self resetUI:[NSDictionary dictionaryWithObjectsAndKeys:@"Updated", DBMUStatusKey, nil]];
  [arp release];
}
