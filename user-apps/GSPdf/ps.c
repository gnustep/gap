...
struct document *psscan(FILE *file)
{
	...
	while (preread || readline(line, sizeof line, file, &position, &line_len)) {
		...
		if (line[0] != '%' || iscomment(line+1, "%EndComments") || line[1] == ' ' 
									|| line[1] == '\t' || line[1] == '\n' || !isprint(line[1])) {
	    break;
		} else if (line[1] != '%') {
	    /* Do nothing */
		...
		} else if (doc->nummedia == NONE && iscomment(line+2, "DocumentMedia:")) {
	    float w, h;
	    doc->media = (struct documentmedia *) malloc(sizeof (struct documentmedia));
	    if (doc->media == NULL) {
				fprintf(stderr, "Fatal Error: Dynamic memory exhausted.\n");
				exit(-1);
	    }
	    ...
	    section_len += line_len;
	    if (doc->nummedia != 0) {
				doc->default_page_media = doc->media;
			}
	    ...
	    while ((cp = gettext(next_char, &next_char))) {
		      ...
		      * PaperSize comments.
		      */
		    	if (strcasecmp(doc->media[doc->nummedia].name, dmp->name) == 0) {
						free(doc->media[doc->nummedia].name);
						doc->media[doc->nummedia].name = (char *)malloc(strlen(dmp->name)+1);
						if (doc->media[doc->nummedia].name == NULL) {
			    		fprintf(stderr, "Fatal Error: Dynamic memory exhausted.\n");
			    		exit(-1);
						}
...
newpage:
    while (DSCcomment(line) && iscomment(line+2, "Page:")) {
	    	...
	    	maxpages++;
	    	doc->pages = (struct page *) realloc(doc->pages, maxpages*sizeof (struct page));
	    	if (doc->pages == NULL) {
					fprintf(stderr, "Fatal Error: Dynamic memory exhausted.\n");
					exit(-1);
	    	}
...
continuepage:
    ...
    while (preread || readline(line, sizeof line, file, &position, &line_len)) {
	    	...
	    	if (!ignore && thispage == nextpage) {
					if (doc->numpages > 0) {
		    		doc->pages[doc->numpages-1].end = position;
		    		doc->pages[doc->numpages-1].len += doc->lentrailer + section_len - line_len;
					} else {
		    		if (doc->endsetup) {
							doc->endsetup = position;
...
void psfree(struct document *doc)
{
	...
	if (doc) {
	    ...
	    if (doc->pages[i].label) {
				free(doc->pages[i].label);
			}
		}
		for (i=0; i<doc->nummedia; i++) {
	    if (doc->media[i].name) {
				free(doc->media[i].name);
			}
		}
		if (doc->title) {
			free(doc->title);
