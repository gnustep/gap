...
/*
  NEXTSTEP 4.0 changes: 9-2-95

  This gnuchess.c is changed from its original.  The changes are mainly the
  additions of function type declarations (for no compiler warnings); as well
  as small changes for casting types and resetting global variables.  

  Its original is saved in gnuchess.save folder.  When the changed gnuchess.c
  has problems, go back to the saved file and recompile.
*/
...
static void  ZeroTTable(), pick(short p1, short p2);
static int search(short side,
		  short ply,
		  short depth,
		  short alpha,
		  short beta,
...
static void  ExaminePosition(), ScorePosition(short side,short *score), OpeningBook(), repetition(short *cnt);
static short evaluate(short side,
		      short xside,
		      short ply,
		      short alpha,
		      short beta);
static short SqValue(short sq,short side);
...
static int   ScoreKPK(short side, short winner, short loser, short king1,short king2, short sq);
  ...
  if (cnt == 1){
    ...
    if (SqAtakd(PieceList[opponent][0],computer)){
      ...
      if (xnode.flags & epmask)
        ...
        UpdateDisplay((int)xnode.f,(int)xnode.t,0,(int)(xnode.flags & cstlmask));
...
void SelectLoop( struct MoveInfo *mi )
{
  ...
  Sdepth++;
  ShowDepth(' ');
  score = search( mi->side, 1, Sdepth, mi->alpha, mi->beta, PrVar, &mi->rpt );
  for (i = 1; i <= Sdepth; i++)
    killr0[i] = PrVar[i];
  if( score < mi->alpha ){
    ShowDepth('-');
    ...
    ZeroTTable();
    score = search( mi->side, 1, Sdepth, -9000, mi->beta, PrVar, &mi->rpt );
  }
  if( score > mi->beta && !(root->flags & exact)){
    ShowDepth('+');
    ExtraTime = 0;
    ZeroTTable();
    score = search( mi->side, 1, Sdepth, mi->alpha, 9000, PrVar, &mi->rpt );
  }
  score = root->score;
  if (!timeout || Sdepth<2)
  ...
  mi->beta = score + Bwindow;
  if(Zscore < score)
    mi->alpha = Zscore - Awindow - zwndw;
  else
    mi->alpha = score - Awindow - zwndw;
}

void SelectMove(short side, short iop)
/*
...
void SelectMoveStart( struct MoveInfo *mi )
{
  ...
  if( mi->iop != 2 )
    player = mi->side;
  if(TCflag){
    if (((TimeControl.moves[mi->side] + 3) - OperatorTime) != 0)
      ResponseTime = (TimeControl.clock[mi->side]) /
		     (TimeControl.moves[mi->side] + 3) -
		     OperatorTime;
    else
      ResponseTime = 0;
    ResponseTime += (ResponseTime*TimeControl.moves[mi->side])/(2*TCmoves+1);
  }
  else
    ResponseTime = Level;
  if( mi->iop == 2 )
  ...
  ExtraTime = 0;
  ExaminePosition();
  ScorePosition( mi->side, &score);
  ShowSidetomove();
  
  if( Sdepth == 0 ){
    ZeroTTable();
    SearchStartStuff( mi->side );
    for (i = 0; i < 8192; i++)
      history[i] = 0;
...
int SelectMoveEnd( struct MoveInfo *mi )
{
  static short score, tempb, tempc, tempsf, tempst;

  score = root->score;
  if( mi->rpt >= 2 || score < -12000 )
    root->flags |= draw;
  if( mi->iop == 2 )
    return(0);
  if (Book == NULL)
  ...
  if (score > -9999 && mi->rpt <= 2){
    MakeMove( mi->side, root, &tempb, &tempc, &tempsf, &tempst );
  ...
  if (TCflag){
    TimeControl.clock[mi->side] -= (et + OperatorTime);
    if (--TimeControl.moves[mi->side] == 0)
      SetTimeControl();
  }
  ...
  if (GameCnt > 238)
    quit = true;
  player = otherside[mi->side];
  Sdepth = 0;
  fflush(stdin);
  
  if( quit || mate || (root->flags & draw) )
    GameEnd( score );
...
struct leaf *node,tmp;
  ...
  for (pnt = pbst = TrPnt[ply]; pnt < TrPnt[ply+1] && best <= beta; pnt++){
    ...
    if (node->score > best && (!timeout || Sdepth < 2)){
      if (depth > 0)
...
void PutInTTable(short side,short score,short depth,short alpha,short beta,unsigned short mv)
/*
